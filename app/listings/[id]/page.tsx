...
const LISTINGS: Record<number, {
  id: number; title: string; condition: string; price: number; original: number;
  icon: string; seller: string; rating: number; reviews: number; badge: string | null;
  description: string; specs: Record<string, string>; sellerJoined: string; sellerSales: number;
}> = {
  1: {
  ...
  2: {
    ...
    description: "MacBook Air M2 in good condition. Light scratches on the bottom lid (not visible when open). Screen is perfect — no dead pixels or backlight issues. Keyboard and trackpad work flawlessly. Battery cycles at 112.",
  ...
  3: {
    ...
    description: "Samsung Galaxy S24 in like-new condition. Purchased 2 months ago, used lightly. No signs of wear. All original accessories included. Unlocked for all carriers.",
  ...
  4: {
    ...
    description: "Sony WH-1000XM5 wireless headphones in excellent condition. Industry-leading noise cancellation. Used for about 8 months. No scratches on the headband or ear cups. Comes with carry case and cables.",
  ...
  5: {
    ...
    description: "iPad Air 5th generation with M1 chip. Good condition with minor wear on the corners. Screen is pristine. WiFi only model. Apple Pencil 2 compatible. Great for drawing, notes, and media.",
  ...
  6: {
    ...
    description: "PlayStation 5 disc edition in like-new condition. Only used for about 3 months. Comes with one DualSense controller, all original cables, and box. No games included. Works perfectly, no issues.",
    ...
    <div style={{ minHeight: "100vh", background: "#0f0f13", color: "#fff", fontFamily: "system-ui, sans-serif" }}>
      ...
      <nav style={{ background: "#18181f", borderBottom: "1px solid #2a2a35", padding: "0 24px", height: 60, display: "flex", alignItems: "center", gap: 16, position: "sticky", top: 0, zIndex: 100 }}>
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
          <button style={{ background: "#a78bfa", border: "none", color: "#fff", fontWeight: 600, fontSize: 14, borderRadius: 999, padding: "8px 20px", cursor: "pointer" }}>Sign up</button>
        </div>
      </nav>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 24px", display: "grid", gridTemplateColumns: "1fr 420px", gap: 40 }}>
        ...
        <div>
          ...
          <div style={{ background: "#18181f", border: "1px solid #2a2a35", borderRadius: 20, height: 380, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 120, marginBottom: 12, position: "relative" }}>
            {thumbs[activeImg]}
            {item.badge && (
              <span style={{ position: "absolute", top: 16, left: 16, background: "#a78bfa", color: "#fff", fontSize: 12, fontWeight: 700, borderRadius: 999, padding: "4px 12px" }}>{item.badge}</span>
            )}
          </div>
          {/* Thumbnails */}
          <div style={{ display: "flex", gap: 10, marginBottom: 32 }}>
            {thumbs.map((t, i) => (
              <div key={i} onClick={() => setActiveImg(i)} style={{ background: "#18181f", border: `2px solid ${activeImg === i ? "#a78bfa" : "#2a2a35"}`, borderRadius: 12, width: 72, height: 72, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 32, cursor: "pointer" }}>
                {t}
              </div>
            ))}
          </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              {Object.entries(item.specs).map(([k, v]) => (
                <div key={k} style={{ background: "#18181f", border: "1px solid #2a2a35", borderRadius: 10, padding: "10px 14px", display: "flex", justifyContent: "space-between" }}>
                  ...
                  <span style={{ color: "#ddd", fontSize: 13, fontWeight: 500 }}>{v}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ position: "sticky", top: 80, alignSelf: "start" }}>
          <div style={{ background: "#18181f", border: "1px solid #2a2a35", borderRadius: 20, padding: 24, marginBottom: 16 }}>
            ...
            <button style={{ width: "100%", background: "linear-gradient(90deg, #a78bfa, #818cf8)", border: "none", color: "#fff", fontWeight: 700, fontSize: 16, borderRadius: 12, padding: "14px 0", cursor: "pointer", marginBottom: 10 }}>
              Buy now — ${item.price}
            </button>
            <button
              onClick={() => setOfferOpen(!offerOpen)}
              style={{ width: "100%", background: "transparent", border: "1px solid #a78bfa", color: "#a78bfa", fontWeight: 600, fontSize: 15, borderRadius: 12, padding: "12px 0", cursor: "pointer", marginBottom: 10 }}>
              Make an offer
            </button>
            <button
              onClick={() => setSaved(!saved)}
              style={{ width: "100%", background: "transparent", border: "1px solid #2a2a35", color: saved ? "#f87171" : "#888", fontWeight: 600, fontSize: 15, borderRadius: 12, padding: "12px 0", cursor: "pointer" }}>
            ...
            {offerOpen && !offerSent && (
              <div style={{ marginTop: 16, background: "#0f0f13", borderRadius: 12, padding: 16, border: "1px solid #2a2a35" }}>
                <p style={{ fontSize: 13, color: "#aaa", marginBottom: 10 }}>Enter your offer price (listing is ${item.price})</p>
                <div style={{ display: "flex", gap: 8 }}>
                  <input
                    type="number"
                    value={offerValue}
                    onChange={(e) => setOfferValue(e.target.value)}
                    placeholder={`e.g. ${item.price - 50}`}
                    style={{ flex: 1, background: "#18181f", border: "1px solid #2a2a35", borderRadius: 8, padding: "8px 12px", color: "#fff", fontSize: 14, outline: "none" }}
                  />
                  <button
                    onClick={() => { if (offerValue) setOfferSent(true); }}
                    style={{ background: "#a78bfa", border: "none", color: "#fff", fontWeight: 600, borderRadius: 8, padding: "8px 16px", cursor: "pointer" }}>
                    Send
                  </button>
                </div>
              </div>
            ...
            {offerSent && (
              <div style={{ marginTop: 16, background: "rgba(74,222,128,0.1)", border: "1px solid rgba(74,222,128,0.3)", borderRadius: 12, padding: 14, textAlign: "center" }}>
                ...
                <p style={{ color: "#666", fontSize: 12 }}>The seller will respond within 24 hours.</p>
              </div>
            )}
          </div>
          ...
          <div style={{ background: "#18181f", border: "1px solid #2a2a35", borderRadius: 20, padding: 20 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
              <div style={{ width: 44, height: 44, borderRadius: "50%", background: "linear-gradient(135deg, #a78bfa, #60a5fa)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 18 }}>
                {item.seller[0]}
              </div>
              <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14 }}>
              <div style={{ background: "#0f0f13", borderRadius: 10, padding: 10, textAlign: "center" }}>
                <div style={{ fontWeight: 700, fontSize: 18, color: "#a78bfa" }}>{item.sellerSales}</div>
                <div style={{ fontSize: 12, color: "#555" }}>Total sales</div>
              </div>
              <div style={{ background: "#0f0f13", borderRadius: 10, padding: 10, textAlign: "center" }}>
                <div style={{ fontWeight: 700, fontSize: 15, color: "#a78bfa" }}>{item.sellerJoined}</div>
                <div style={{ fontSize: 12, color: "#555" }}>Member since</div>
              </div>
            </div>
            <button style={{ width: "100%", background: "transparent", border: "1px solid #2a2a35", color: "#ccc", fontWeight: 600, fontSize: 14, borderRadius: 10, padding: "10px 0", cursor: "pointer" }}>
              💬 Message seller
            </button>
          </div>
          ...
          <div style={{ marginTop: 16, display: "flex", flexDirection: "column", gap: 8 }}>
            {[["🛡️", "Buyer Protection", "Your payment is secured until you confirm receipt"],
              ["🚚", "Fast Shipping", "Most sellers ship within 1-2 business days"],
              ["↩️", "Easy Returns", "30-day return policy on all listings"]].map(([icon, title, desc]) => (
              <div key={title as string} style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                <span style={{ fontSize: 18 }}>{icon}</span>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "#ddd" }}>{title as string}</div>
                  <div style={{ fontSize: 12, color: "#555" }}>{desc as string}</div>
                </div>
              </div>
            ))}
          </div>
